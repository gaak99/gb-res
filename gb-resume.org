...
It was run like a startup using Agile methodology and JIRA/Confluence/Stash(git) was used for all the projects.
...
A teammate wrote a user/customer DB that the other services accessed using SQLAlchemy.
