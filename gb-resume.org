...
The service was based on Openstack Swift[fn:3] object storage code (Python).

Swift runs on a Ubuntu cluster (we had one cluster in Salt Lake City and a second one in Oklahoma City).

Swift is open source and thousands of lines of Python 2 and is heavily object oriented.
And the code tends to be of high quality as it has many eyes of Pythen experts for code review.
